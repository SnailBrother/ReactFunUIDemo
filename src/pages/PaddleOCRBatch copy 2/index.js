...
const OcrTextExtraction = () => {
  ...
  const [filePreviews, setFilePreviews] = useState([]); // 预览列表：{type, url, name, pageNumber}
  ...
  const [pageResults, setPageResults] = useState([]); // 存储每页的结果和预览配对
  const [highlightedPage, setHighlightedPage] = useState(null); // 高亮当前查看的页面
  ...
  const removeFile = (index) => {
    ...
    if (!remainingFromSameFile) {
      // 如果该文件的所有预览都被移除了，从selectedFiles中移除
          ...
          <div className={styles.uploadContent}>
            ...
            {filePreviews.length > 0 && (
              <div className={styles.uploadedFilesInfo}>
                已上传 {selectedFiles.length} 个文件，共 {filePreviews.length} 页
              </div>
            )}
          </div>
        </div>
